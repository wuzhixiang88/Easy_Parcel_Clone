...
io.on("connection", (socket) => {
  ...
  socket.on(NEW_CHAT_MESSAGE_EVENT, async (data) => {
    ...
    if (!roomExists) {
        await chatModel.create({ parcelID: roomId, chatlog: [{ senderID: senderId, body: body}] })
    } else {
        await chatModel.updateOne({ parcelID: roomId}, { $push: { chatlog: { senderID: senderId, body: body }}})
    }
